...
import ConvertAction, { ConvertedImage } from './ConvertAction';
import imageCompression from 'browser-image-compression';
// Import the mocked components (Vitest hoists vi.mock)
import { Select as MockedSelect, RadioGroup as MockedRadioGroup, Checkbox as MockedCheckbox } from '@/components/ui/select'; // This path will be resolved by alias/mock

// Mock browser-image-compression
vi.mock('browser-image-compression', () => ({
  default: vi.fn(),
}));
const mockImageCompression = imageCompression as vi.MockedFunction<typeof imageCompression>;
...
describe('ConvertAction Component', () => {
  ...
  const sampleImages = [new File(['dummy_content_1'], 'image1.jpg', { type: 'image/jpeg' })];
  const sampleImagesPNG = [new File(['dummy_content_png'], 'imagePNG.png', { type: 'image/png' })];
  ...
  beforeEach(() => {
    ...
    mockImageCompression.mockImplementation(async (file, options) => {
      ...
      return new File(['compressed_dummy'], fileName, { type: options?.fileType || file.type });
    });
  });

  const renderComponent = (images = sampleImages) => {
    return render(<ConvertAction images={images} setConverted={mockSetConverted} />);
  };

  it('renders correctly with initial default UI elements', () => {
    renderComponent();
    expect(screen.getByLabelText(/Max Size \(MB\)/i)).toBeInTheDocument();
    expect(screen.getByLabelText(/Max Width\/Height \(px\)/i)).toBeInTheDocument();
    ...
    expect(screen.getByLabelText(/Aggressive \(Smaller Size\)/i)).toBeInTheDocument(); // Mocked RadioGroupItem contains label and input
    expect(screen.getByLabelText(/High Quality \(Larger Size\)/i)).toBeInTheDocument();

    expect(screen.getByText(/Keep EXIF Data/i)).toBeInTheDocument(); // Label is separate from checkbox mock
    expect(screen.getByTestId('global-mock-checkbox-input-keepExif')).toBeInTheDocument(); // Check actual input in mock
  ...
  it('calls imageCompression with default options on conversion', async () => {
    renderComponent();
    const convertButton = screen.getByRole('button', { name: /Convert \(1\)/i });
    await act(async () => {
  ...
  describe('Output Format Conversion', () => {
    it.each([
    ...
    ])('converts to $format correctly', async ({ format, fileType, expectedExtension }) => {
      await act(async () => {
        renderComponent(sampleImagesPNG);
      });
      
      const itemToClick = screen.getByTestId(`global-mock-select-item-${fileType}`);
      ...
      expect(mockSetConverted).toHaveBeenCalledWith(
        expect.arrayContaining([
          expect.objectContaining({ image_name: `Reduced_imagePNG.${expectedExtension}` }),
        ])
      );
    });
    ...
    it('keeps original format if "Keep Original Format" is selected', async () => {
      ...
      const originalItem = screen.getByTestId('global-mock-select-item-original');
      const selectTrigger = screen.getByTestId('global-mock-select-trigger');
  ...
  describe('Compression Mode', () => {
    it('uses High Quality settings when selected', async () => {
      ...
      const highQualityRadioContainer = screen.getByTestId('global-mock-radiogroup-item-container-high');
      await act(async () => {
    ...
    it('uses Aggressive (lossy) settings by default and when selected', async () => {
      ...
      let convertButton = screen.getByRole('button', { name: /Convert \(1\)/i });
      await act(async () => {
        await userEvent.click(convertButton);
      });
      ...
      const aggressiveRadioContainer = screen.getByTestId('global-mock-radiogroup-item-container-lossy');
      await act(async () => {
  ...
  describe('Interaction of Options', () => {
    it('combines WEBP, High Quality, and Keep EXIF correctly', async () => {
      ...
      // Check Keep EXIF
      const keepExifCheckbox = screen.getByTestId('global-mock-checkbox-input-keepExif'); // Click input directly
       await act(async () => {
        await userEvent.click(keepExifCheckbox);
      });
  ...
  describe('Local Storage Persistence', () => {
    it('loads and saves MaxSizeMB from/to localStorage', async () => {
      ...
      expect(screen.getByLabelText<HTMLInputElement>(/Max Size \(MB\)/i).value).toBe('0.5'); 

      const inputElement = screen.getByLabelText<HTMLInputElement>(/Max Size \(MB\)/i);
      await act(async () => { 
      ...
      expect(localStorageMock.getItem('compressionMaxSizeMB')).toBe(JSON.stringify(0.7));
    });

    it('loads and saves Output Format from/to localStorage', async () => {
      localStorageMock.setItem('compressionOutputFormat', JSON.stringify('image/png'));
      let unmount: () => void;
      await act(async () => {
      ...
      expect(localStorageMock.getItem('compressionOutputFormat')).toBe(JSON.stringify('image/webp'));

      await act(async () => { unmount!(); });
      localStorageMock.setItem('compressionOutputFormat', JSON.stringify('image/webp'));
      await act(async () => {
        render(<ConvertAction images={sampleImages} setConverted={mockSetConverted} />);
      });
    ...
    it('loads and saves Compression Mode from/to localStorage', async () => {
      localStorageMock.setItem('compressionQualityMode', JSON.stringify('high'));
      let unmountFn: () => void;
      await act(async () => {
        const { unmount } = renderComponent();
        unmountFn = unmount;
      });
      expect(screen.getByTestId<HTMLInputElement>('global-mock-radiogroup-input-high').checked).toBe(true);
      
      const aggressiveRadioContainer = screen.getByTestId('global-mock-radiogroup-item-container-lossy');
      await act(async () => {
        await userEvent.click(aggressiveRadioContainer);
      });
      expect(localStorageMock.getItem('compressionQualityMode')).toBe(JSON.stringify('lossy'));
      
      await act(async () => { unmountFn!(); });
      localStorageMock.setItem('compressionQualityMode', JSON.stringify('lossy'));
      await act(async () => { renderComponent(); });
      expect(screen.getByTestId<HTMLInputElement>('global-mock-radiogroup-input-lossy').checked).toBe(true);
    });
    ...
    it('loads and saves Keep EXIF from/to localStorage', async () => {
      ...
      expect(screen.getByTestId<HTMLInputElement>('global-mock-checkbox-input-keepExif').checked).toBe(true);
      
      const keepExifCheckboxInput = screen.getByTestId('global-mock-checkbox-input-keepExif');
      await act(async () => {
        await userEvent.click(keepExifCheckboxInput);
      });
      expect(localStorageMock.getItem('compressionKeepExif')).toBe(JSON.stringify(false));

      await act(async () => { unmountFn!(); });
      localStorageMock.setItem('compressionKeepExif', JSON.stringify(false));
      await act(async () => { renderComponent(); });
      expect(screen.getByTestId<HTMLInputElement>('global-mock-checkbox-input-keepExif').checked).toBe(false);
    });
  });
});
